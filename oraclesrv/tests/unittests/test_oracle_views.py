...
from oraclesrv.views import get_user_info_from_adsws, cleanup, list_tmps, list_multis, get_the_reader, read_history, \
    docmatch, verify_the_function
from oraclesrv.score import clean_metadata


class test_views(TestCaseDatabase):

    def test_readhist_endpoint_post(self):
        """
        Test POST for readhist endpoint when no optional param passed in, so default is returned
        """
    ...
    def test_readhist_endpoint_get(self):
        """
        Test GET endpoint for readhist endpoint with default params
        """
        # the mock is for solr call
        with mock.patch.object(self.current_app.client, 'get') as get_mock:
            get_mock.return_value = mock_response = mock.Mock()
    ...
    def test_docmatch_endpoint_metadata(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint using metadata abstract/title to query solr
        """
    ...
    def test_docmatch_endpoint_no_result_from_solr_metadata(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint having no result from solr when solr queried with abstract/title
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    def test_docmatch_endpoint_with_doi_and_collaboration(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint having doi and being a collaboration
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
        ...
        with mock.patch.object(self.current_app.client, 'get') as get_mock:
            ...
            # also test docmatch endpoint (without saving)
    ...
    def test_docmatch_endpoint_with_doi_not_matching(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint having erroneous doi
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    def test_docmatch_endpoint_with_accented_author(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint having accented author
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    def test_docmatch_endpoint_no_abstract_solr_record(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint when there is no abstract that title is quarried
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    def test_docmatch_endpoint_matching_thesis(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint having thesis
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    def test_docmatch_endpoint_no_result_from_solr_thesis(self):
        """
        Test docmatch endpoint having no result from solr when solr queried with thesis information
        """
        with mock.patch.object(self.current_app.client, 'get') as get_mock:
            get_mock.return_value = mock_response = mock.Mock()
            mock_response.status_code = 200
    ...
    def test_clean_data(self):
        """
        Test clean_metadata function that cleans up abstract and title
        """
    ...
    def test_docmatch_endpoint_no_abstract_source(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint using metadata abstract/title to query solr, when there is no abstract, and we found a match
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    @mock.patch("oraclesrv.views.query_docmatch")
    def test_query_endpoint(self, mock_query_docmatch):
        """
        Test query endpoint with and without params passing in
        """
        in_db = [
                    ['2018arXiv180310259Z', '2017PhDT........67Z', 0.8730186],
                    ['2017arXiv171011147R', '2018Natur.556..473R', 0.8745491],
                    ['2019arXiv190500882A', '2019JHEP...06..121A', 0.8960806],
                    ['2019arXiv190804722C', '2019JHEP...10..244S', 0.8865355],
                    ['2020arXiv200210896G', '2020A&A...635A.193G', 0.8988905],
                    ['2019arXiv190802041G', '2020Icar..33613407G', 0.8766192],
                    ['2022arXiv221016332G', '2023A&A...669A...7G', 0.9859276]
                ]
        mock_query_docmatch.return_value = (in_db, 200)

        # Test with no params
        r = self.client.post(path='/query')
        result = json.loads(r.data)
        self.assertDictEqual(result, {'params': {'rows': 2000, 'start': 0, 'date_cutoff': '1972-01-01 00:00:00+00:00'}, 'results': in_db})
        ...
        # test when days param is included
        with mock.patch("oraclesrv.views.get_date") as mock_get_date:
            mock_get_date.return_value = datetime(2024, 12, 9, 0, 0, 0)

            r = self.client.post(path='/query', data=json.dumps({'days': 30}))
            result = json.loads(r.data)
            self.assertDictEqual(result, {'params': {'rows': 2000, 'start': 0, 'days': 30, 'date_cutoff': '2024-11-09 00:00:00'}, 'results': in_db})

    @mock.patch('oraclesrv.utils.query_eprint_bibstem')
    def test_get_match_for_doi_in_pubnote(self, mock_query_eprint_bibstem):
        """
        Test docmatch endpoint using metadata doi in pubnote to query solr
        """
        # mock the eprint_bibstem patterns
        mock_query_eprint_bibstem.return_value = (
            [
    ...
    @mock.patch("oraclesrv.views.current_app.client.get")
    def test_get_user_info_from_adsws(self, mock_get):
        """
        Test get_user_info_from_adsws
        """
        # test when the exception happens
        mock_get.side_effect = Exception("Test exception")
        with self.assertRaises(Exception):
            get_user_info_from_adsws("invalid_param")

    @mock.patch("oraclesrv.views.get_user_info_from_adsws")
    def test_get_the_reader(self, mock_get_user_info):
        """
        Test get_the_reader
        """
        # get userid from the valid account data

        # mock the request object with headers
        mock_request = mock.Mock()
        mock_request.headers = {"Authorization": "Bearer valid_token"}
        mock_request.cookies = {}

        # mock the return value of get_user_info_from_adsws
        mock_get_user_info.return_value = {"hashed_client_id": "1234567890abcdef1234567890abcdef"}

        reader = get_the_reader(mock_request, payload={})

        mock_get_user_info.assert_called_once_with("valid_token")
        self.assertEqual(reader, "1234567890abcdef")


        # remove Authorization and it the cookies to test getting userid from session
        mock_request.headers = {}
        mock_request.cookies = {"session": "valid_session"}

        # mock the return value of get_user_info_from_adsws
        mock_get_user_info.side_effect = [
            {"hashed_client_id": "1234567890abcdef1234567890abcdef"},  # For Authorization token
            {"hashed_client_id": "fedcba0987654321fedcba0987654321"}  # For session
        ]

        reader = get_the_reader(mock_request, payload={})

        mock_get_user_info.assert_any_call("valid_session")
        self.assertEqual(reader, "1234567890abcdef")

    @mock.patch("oraclesrv.views.get_solr_data_recommend")
    @mock.patch("oraclesrv.views.return_response")
    def test_read_history_with_bibcodes(self, mock_return_response, mock_get_solr_data):
        """
        Test read_history
        """
        # mock the return value of get_solr_data_recommend
        mock_get_solr_data.return_value = (["2024TEST..........S", "2024TEST..........G"], "test_query", 200)

        # mock payload, function, and reader
        payload = {"sort": "entry_date"}
        function = "similar"
        reader = "test_reader"

        read_history(payload, function, reader)

        mock_get_solr_data.assert_called_once_with("similar", "test_reader", 5, "entry_date", 5, 10)
        mock_return_response.assert_called_once_with(results={"bibcodes": "2024TEST..........S,2024TEST..........G", "query": "test_query"}, status_code=200)

    @mock.patch("oraclesrv.views.request.get_json", side_effect=Exception)
    def test_docmatch_without_payload(self, mock_request):
        """
        Test docmatch without a valid payload
        """
        response = docmatch(save=True)
        self.assertEqual(response.status_code, 400)
        self.assertIn("error", json.loads(response.data))

    @mock.patch("oraclesrv.views.get_the_reader")
    @mock.patch("oraclesrv.views.read_history")
    def test_read_history_with_no_params(self, mock_read_history, mock_get_the_reader):
        """
        Test read_history_get when function and reader are None passed in, but they are part of payload
        """
        # mock read_history
        mock_read_history.return_value = jsonify({"bibcodes": "2024TEST..........S,2024TEST..........G", "query": "test_query"}), 200

        with mock.patch.object(self.current_app.client, 'get') as get_mock:
            r= self.client.get(path='/readhist/None/None', query_string={"function": "similar", "reader": "0000000000000000"})
            self.assertEqual(r.status_code, 200)
            results = json.loads(r.data)
            self.assertEqual(results['query'], "test_query")
            self.assertEqual(results['bibcodes'], "2024TEST..........S,2024TEST..........G")

        # test when get_the_reader returns None
        mock_get_the_reader.return_value = None

        with mock.patch.object(self.current_app.client, 'get') as get_mock:
            r= self.client.get(path='/readhist/None/None', query_string={"function": "similar", "reader": "0000000000000000"})
            self.assertEqual(r.status_code, 400)

    @mock.patch("oraclesrv.views.Parse")
    def test_add_and_delete_when_parse_error(self, mock_parse):
        """
        Test the /add and /delete endpoints when a ParseError is raised.
        """
        # Mock Parse to raise a ParseError
        mock_parse.side_effect = ParseError("Mocked parse error")

        r = self.client.put(path='/add', json=[{"bibcode": "test", "other_data": "value"}])
        self.assertEqual(r.status_code, 400)
        self.assertDictEqual(r.json, {'error': 'unable to extract data from protobuf structure -- Mocked parse error'})

        r = self.client.delete(path='/delete', json=[{"bibcode": "test", "other_data": "value"}])
        self.assertEqual(r.status_code, 400)
        self.assertDictEqual(r.json, {'error': 'unable to extract data from protobuf structure -- Mocked parse error'})

    def test_add_and_delete_payload_exception(self):
        """
        Test the /add and /delete endpoints with payload parsing throwing exception
        """
        with mock.patch("oraclesrv.views.request.get_json", side_effect=Exception("Mocked exception")):
            r = self.client.put(path='/add', content_type='application/x-www-form-urlencoded')
            self.assertEqual(r.status_code, 400)
            self.assertDictEqual(r.json, {'error': 'no payload received'})

            r = self.client.delete(path='/delete', content_type='application/x-www-form-urlencoded')
            self.assertEqual(r.status_code, 400)
            self.assertDictEqual(r.json, {'error': 'no payload received'})
